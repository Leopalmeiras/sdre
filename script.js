(() => {
  ...
  function setupLeads() {
    ...
    byId('exportLeadsPdf')?.addEventListener('click', exportLeadsPdfProfessional);
  ...
  function setupTarefas() {
    ...
    byId('exportTarefasPdf')?.addEventListener('click', () =>
      exportPdfTable({
        filename: 'relatorio-tarefas.pdf',
        title: 'Relatório de Tarefas',
        head: [tarefasHeaders()],
        body: tarefasRows(getTarefasAtualizadas()),
  ...
  function exportLeadsPdfProfessional() {
    const head = [[
      'Nome',
      'Empresa',
      'E-mail',
    ...
    const body = getLeads().map((lead) => {
      const { datePart, timePart } = getLeadDateTimeParts(lead);
      return [
        lead.nome || 'Não informado',
        lead.empresa || 'Não informado',
        lead.email || 'Não informado',
        lead.telefone || 'Não informado',
        lead.cnpj || 'Não informado',
        lead.origem || 'Não informado',
        lead.classificacao || 'Não informado',
        lead.observacao || 'Não informado',
        datePart,
        timePart,
      ];
    });
  ...
  function exportPdfTable({ filename, title, head, body, emptyMessage }) {
    const jsPDFNamespace = window.jspdf;
    if (!jsPDFNamespace?.jsPDF) {
      alert('Biblioteca de PDF não carregada. Recarregue a página e tente novamente.');
      return;
    }
    const doc = new jsPDFNamespace.jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });
    ...
    doc.setFontSize(10);
    doc.text(`Data de geração: ${generatedAt}`, 14, 23);

    const contentBody = body.length ? body : [[emptyMessage]];
    const contentHead = body.length ? head : [['Informação']];
  ...
  function getLeadDateTimeParts(lead) {
    const iso = lead?.createdAtISO;
    if (iso) {
      const date = new Date(iso);
      if (!Number.isNaN(date.getTime())) {
        const datePart = new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' }).format(date);
        const timePart = new Intl.DateTimeFormat('pt-BR', { hour: '2-digit', minute: '2-digit', hour12: false }).format(date);
        return { datePart, timePart };
      }
    }
    if (lead?.createdAtFormatted && lead.createdAtFormatted.includes(' ')) {
      const [datePart, timePart] = lead.createdAtFormatted.split(' ');
      return { datePart: datePart || 'Não informado', timePart: timePart || 'Não informado' };
    }
    return { datePart: 'Não informado', timePart: 'Não informado' };
  }
